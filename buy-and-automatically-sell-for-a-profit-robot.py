...
def buy_stocks(bought_stocks, stocks_to_buy, buy_sell_lock):
    ...
    for symbol in stocks_to_buy:
        ...
        # It is also important to check that the current price is less than the opening price by a
        # specific, and strict percentage before buying the stock.
        # This check is with the profit_buy_price_setting.
    ...
    for symbol, (bought_price, purchase_date) in bought_stocks.items():
        ...
        if bought_date_str < today_date_str:    # keep under the "s" in "for symbol"
            ...
            if current_price >= bought_price * 1.025:  # keep this under the "o" in "bought"
                ...
                with open(csv_filename, mode='a', newline='') as csv_file:
                    csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                    csv_writer.writerow({'Date': current_time_str, 'Sell': 'Sell', 'Quantity': qty, 'Symbol': symbol, 'Price Per Share': current_price})
